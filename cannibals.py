...
class ButtonActions:
    ...
    def OnPlay(ui, game):
        '''prepare game'''
        ui.createGameUI()
        game.createBackground('background.png')
...
class Button(UIField):
    ...
    def __init__(self, text, center, onClick=None, z=5):
        ...
        self.rect = self.box.get_rect()
        self.rect.center = center
        self.onClick = onClick if onClick != \
            None else ButtonActions.doNothing
...
class GameObject():
    ...
    def moveTo(self, endPos, time):
        ...
        Game.asyncTasks.append(
            {'obj': self, 'func': 'moveToAsync',
             'args': {'startPos': self.rect.center,
                      'endPos': endPos,
...
class Raft(GameObject):
    ...
    def __init__(self):
        self.img = pygame.image.load('raft.png')
        self.img = self.img.convert_alpha()
        self.rect = self.img.get_rect()
        self.pos = {'right': (830, 650), 'left': (430, 650)}
        self.rect.center = (830, 650)
        Raft.slot1 = {'person': None, 'pos': (
            self.rect.left + 50, self.rect.bottom - 150)}
        Raft.slot2 = {'person': None, 'pos': (
            self.rect.right - 50, self.rect.bottom - 150)}
        self.animTick = 0
        Raft.state = 'idle'
        Raft.side = 'right'
        self.otherside = 'left'
            ...
            for task in Game.asyncTasks:
                if task['args'] == args:
                    ...
                    if isinstance(a, Cannibal) and b is None:
                        ...
                        graphstate = gameGraph[graphstate]['m']
                    elif isinstance(a, Missionary) and \
                            isinstance(b, Missionary):
    ...
    def anim(self):
        ...
        if self.state == 'hovered':
            self.img = pygame.image.load('rhovered.png')
            self.img = self.img.convert_alpha()
            Raft.state = 'idle'
    ...
    def onClick(self):
        ...
        if ((c1 != None and c1.state == 'idle' and
             c2 is None and self.state == 'idle') or
            (c2 != None and c2.state == 'idle' and
             c1 is None and self.state == 'idle') or
            (c1 != None and c2 != None and
             c1.state == c2.state == self.state == 'idle')):
            Game.asyncTasks.append(
                {'obj': self, 'func': 'moveBoat', 'args':
                 {'startTime': pygame.time.get_ticks(), 'duration': 3}})
...
class Cannibal(GameObject):
    ...
    def __init__(self, slot):
        ...
        self.side = 'right'
        self.slot = slot
        self.pos = {'right': (1050 + 70 * slot, 350 + 115 * slot),
                    'left': (180 - 50 * slot, 400 + 100 * slot)}
        self.animCounter = 0
        self.idleSheet = tuple(pygame.image.load(
            'idle' + str(i) + '.png')for i in range(2))
        self.moveSheet = tuple(pygame.image.load(
            'move' + str(i) + '.png')for i in range(2))
        self.rect = self.img.get_rect()
    ...
    def anim(self):
        ...
        if self.state == 'idle':
            if self.animTick % Game.frameRate == 0:
                self.img = self.idleSheet[self.animCounter %
                                          len(self.idleSheet)]
                self.img = self.img.convert_alpha()
                self.animCounter += 1
        if self.state == 'move':
            if self.animTick % (Game.frameRate / 8) == 0:
                self.img = self.moveSheet[self.animCounter %
                                          len(self.moveSheet)]
                self.img = self.img.convert_alpha()
                self.animCounter += 1
        if self.state == 'hovered':
            self.img = pygame.image.load('hovered.png')
            self.img = self.img.convert_alpha()
            self.state = 'idle'
            self.animTick = -1
            self.animCounter = 0
    ...
    def onClick(self):
        if self.state == 'idle' and Raft.state == 'idle':
            if Raft.slot1['person'] == self:
                ...
                self.moveTo(self.pos[self.side], 1)
            elif Raft.slot1['person'] is None and \
                    self.side == Raft.side:
                self.moveTo(Raft.slot1['pos'], 1)
                Raft.slot1['person'] = self
            elif Raft.slot2['person'] is None and \
                    self.side == Raft.side:
...
class Missionary(GameObject):
    ...
    def __init__(self, slot):
        ...
        self.side = 'right'
        self.slot = slot
        self.pos = {'right': (900 + 50 * slot, 375 + 115 * slot),
                    'left': (330 - 50 * slot, 425 + 100 * slot)}
        self.animCounter = 0
        self.idleSheet = tuple(pygame.image.load(
            'midle' + str(i) + '.png')for i in range(2))
        self.moveSheet = tuple(pygame.image.load(
            'mmove' + str(i) + '.png')for i in range(2))
        self.rect = self.img.get_rect()
        self.rect.center = self.pos['right']
    ...
    def anim(self):
        ...
        if self.state == 'move':
            if self.animTick % (Game.frameRate / 8) == 0:
                ...
                self.animCounter += 1
        if self.state == 'hovered':
            self.img = pygame.image.load('mhovered.png')
            self.img = self.img.convert_alpha()
            self.state = 'idle'
            self.animTick = -1
            self.animCounter = 0
...
class Game:
    ...
    def __init__(self, res):
        ...
        Game.missionaries = []
        Game.asyncTasks = []
        Game.FontBIG = pygame.font.Font('font.ttf', 120)
        Game.Font = pygame.font.Font('font.ttf', 40)
        Game.raft = None
        self.dimSur = None
    ...
    def createBackground(self, path):
        ...
        backGnd.rect = pygame.Rect(
            0, 0, backGnd.rect.width, backGnd.rect.height)
    ...
    def dimScreen(self, time):
        ...
        Game.asyncTasks.append(
            {'obj': self, 'func': 'dim',
             'args': {'sur': sur,
                      'startAlpha': 255, 'endAlpha': 0,
                      'startTime': pygame.time.get_ticks(), 'duration': time}})
...
class UI:
    ...
    def drawAgain():
        ...
        Game.asyncTasks.append(
            {'obj': UI, 'func': 'drawAgainAsync',
             'args': pygame.time.get_ticks()})

    @staticmethod
    def drawAgainAsync(sTime):
        if (pygame.time.get_ticks() - sTime) > 2000:
            UI.objs.append(Button('Again?', (640, 560),
                                  ButtonActions.startAgain))
            for task in Game.asyncTasks:
                if task['args'] == sTime:
                    Game.asyncTasks.remove(task)
...
gameGraph = {
    '-cccmmmb': {'c': 'bc-ccmmm', 'm': 'bm-cccmm', '2c': 'bcc-cmmm',
                 '2m': 'bmm-cccm', 'cm': 'bcm-ccmm'},
    'm-cccmmb': FAILURE,
    'mm-cccmb': FAILURE,
    'mmm-cccb': {'c': 'bcmmm-cc', '2c': 'bccmmm-c'},
    'c-ccmmmb': {'c': 'bcc-cmmm', '2c': 'bccc-mmm',
                 'm': 'bcm-ccmm', '2m': 'bcmm-ccm', 'cm': 'bccm-cmm'},
    'cc-cmmmb': {'c': 'bccc-mmm', 'm': 'bccm-cmm',
                 '2m': 'bccmm-cm', 'cm': 'bcccm-mm'},
    'ccc-mmmb': {'m': 'bcccm-mm', '2m': 'bcccmm-m'},
    'cm-ccmmb': {'c': 'bccm-cmm', 'm': 'bcmm-ccm',
                 '2c': 'bcccm-mm', '2m': 'bcmmm-cc', 'cm': 'bccmm-cm'},
    'ccm-cmmb': FAILURE,
    'cccm-mmb': FAILURE,
    'cmm-ccmb': FAILURE,
    ...
    'bcccm-mm': FAILURE,
    'bcmm-ccm': FAILURE,
    'bccmm-cm': {'c': 'cmm-ccmb', 'm': 'ccm-cmmb',
                 '2c': 'mm-cccmb', '2m': 'cc-cmmmb', 'cm': 'cm-ccmmb'},
    'bcccmm-m': FAILURE,
    'bcmmm-cc': {'c': 'mmm-cccb', 'm': 'cmm-ccmb',
                 '2m': 'cm-ccmmb', 'cm': 'mm-cccmb'},

    'bccmmm-c': {'c': 'cmmm-ccb', 'm': 'ccbb-cmb',
                 '2c': 'mmm-cccb', '2m': 'ccm-cmmb', 'cm': 'cmm-ccmb'},
    'bcccmmm-': WIN,
